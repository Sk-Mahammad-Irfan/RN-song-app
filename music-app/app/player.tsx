...
export default function PlayerScreen() {
  ...
  const pendingSeekRatio = useRef(0);
  ...
  const throttledSeek = useCallback((ratio: number) => {
    pendingSeekRatio.current = ratio;
    const now = Date.now();
    if (now - lastSeekCallTime.current > 150) {
      ...
      >
        ...
        >
          <Text style={ { color: isSeeking ? '#fff' : '#8A8AA3', fontSize: 11, width: 38 } }>
            { formatTime(isSeeking ? pendingSeekRatio.current * totalDuration : currentTime) }
          </Text>
          ...
          <View
            ...
            onResponderMove={ (e) => {
              const ratio = getTouchRatio(e);
              seekAnim.setValue(ratio); // instant, no re-render — smooth
              throttledSeek(ratio);     // real-time audio scrub, throttled
            } }
            onResponderRelease={ async (e) => {
              const ratio = getTouchRatio(e);
              seekAnim.setValue(ratio);
              pendingSeekRatio.current = ratio;
              await seekTo(ratio); // final precise seek on release
              setIsSeeking(false);
            } }
